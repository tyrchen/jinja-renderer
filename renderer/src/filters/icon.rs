...
pub fn icon(s: Cow<'_, str>, class: Option<Cow<'_, str>>) -> Result<String> {
    let class = class.as_ref().map(|s| s.as_ref()).unwrap_or_default();
    let attrs = IconAttrs::default().class(class).fill("currentColor");
...
mod tests {
    ...
    #[test]
    fn icon_should_work() {
        insta::assert_snapshot!(icon("github".into(), Some("test".into())).unwrap());
    }
}
